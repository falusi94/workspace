# From: https://github.com/oh-my-fish/theme-agnoster
# name: Agnoster
...
function prompt_arch -d "display the acrhitecture if it's intel"
  if test (arch) = "i386"
    set bg 2B66AF
    set fg FFF
    set fgi 5AC4F8

    if [ "$current_bg" != 'NONE' -a "$bg" != "$current_bg" ]
      set_color $current_bg -b $bg
      echo -n "$segment_separator "
    end
